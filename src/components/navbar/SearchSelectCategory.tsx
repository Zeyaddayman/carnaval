    ...
    return (
        <Listbox value={selectedCategory} onChange={setSelectedCategory}>
            <div className="relative w-fit flex-1">
                <ListboxButton className={`relative h-10 w-full flex justify-between items-center gap-1 cursor-default rounded-s-md bg-input py-3 px-2 border border-border border-e-0 focus-visible:outline-none focus-visible:ring-3 ${searchInputFocused ? "ring-3 ring-primary" : null} focus-visible:ring-primary sm:text-sm/6 transition-[color,box-shadow]`}>
                ...
                >
                    {categories.map((category) => (
                        <ListboxOption
                            key={category.value}
                            value={category}
                            className="group relative flex items-center justify-between gap-2 cursor-default select-none py-1 ps-1 pe-3 text-gray-900 data-focus:bg-primary data-focus:text-white"
                        >
