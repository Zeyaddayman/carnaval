    ...
    return (
        <Listbox value={selected} onChange={setSelected}>
            <div onClick={handlePlacement} ref={selectRef} className="relative min-w-60 max-w-70">
                <ListboxButton className="relative w-full cursor-default rounded-md bg-white border border-border py-3 ps-3 pe-10 text-start shadow-sm focus-visible:outline-none focus-visible:ring-3 focus-visible:ring-primary sm:text-sm/6">
                    <span className="ms-3 block truncate">{title} {selected.label}</span>
                ...
                >
                    {options.map((option) => (
                        <ListboxOption
                            key={option.value}
                            value={option}
                            className="group relative cursor-default overflow-y-hidden select-none py-3 ps-3 pe-9 text-gray-900 data-focus:bg-primary data-focus:text-white"
                        >
                            <span className="ms-3 block truncate font-normal group-data-selected:font-semibold">
                                {option.label}
                            </span>

                            <span className="absolute inset-y-0 end-0 flex items-center pe-4 text-primary group-data-focus:text-white [.group:not([data-selected])_&]:hidden">
